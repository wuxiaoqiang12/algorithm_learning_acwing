...
\begin{titledbox}{AcWing 835. Trie字符串统计}
    ...
    \inlinecode{I x} 向集合中插入一个字符串 $x$；

    \inlinecode{Q x} 询问一个字符串在集合中出现了多少次。
...
\begin{titledbox}{AcWing 836. 合并集合}
    ...
    对于每个询问指令 \inlinecode{Q a b}，都要输出一个结果，如果 $a$ 和 $b$ 在同一集合内，则输出 Yes，否则输出 No。每个结果占一行。
...
\begin{titledbox}{AcWing 837. 连通块中点的数量}
    ...
    \inlinecode{C a b}，在点 $a$ 和点 $b$ 之间连一条边，$a$ 和 $b$ 可能相等；

    \inlinecode{Q1 a b}，询问点 $a$ 和点 $b$ 是否在同一个连通块中，$a$ 和 $b$ 可能相等；

    \inlinecode{Q2 a}，询问点 $a$ 所在连通块中点的数量；
...
用一维数组来模拟一个堆，起始坐标从 \inlinecode{1} 开始，因为 \inlinecode{x} 节点的左儿子是 \inlinecode{2x} 右儿子是 \inlinecode{2x + 1} ，不能用0开始。几种操作的实现如下：
...
此法将从哈希后的那个位置开始查找，如果已经有了不是自己的人占用该位置则继续向后寻找，直到找到一个空位置，返回该位置。此法中Find函数将返回 \inlinecode{x}应该存放的位置。此法仅需开一个数组即可，通常来讲要开数据规模的两倍到三倍，以防buffer overflow。
...
\begin{information}
    注意：此处将字符串存储在下标为1的地方开始。是因为这里将字符串看作是 \inlinecode{P} 进制数，如果从0开始会将同一个字符构成的字符串都算做相同的值，会有问题。
